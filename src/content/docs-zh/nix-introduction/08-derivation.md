...
Nix 构建软件包时使用的是一种被称为 **Derivation（派生）** 的东西。

简单来说，Derivation 就是一份描述软件包构建步骤的超严格「配方」。它包含了交给构建系统的输入、产物的输出目标 Store 路径等信息。本章我们来看看 Derivation 的低层表示形式——**store derivation**。

## store derivation

store derivation 指的是存放在 Nix Store（仓库）中的 `.drv` 文件^[[11. Glossary - Nix Reference Manual](https://nixos.org/manual/nix/stable/glossary.html?highlight=store%20derivation#glossary)]。与普通软件包一样，store derivation 也是一个 Store 对象，同样拥有唯一的 Store 路径。

我们来看看 `hello` 软件包的 store derivation。`hello` 命令是一个用 C 语言编写的简单程序，功能只是向标准输出打印 `Hello, world!`。
下面是笔者环境中 `hello` 软件包的 store derivation 以 JSON 格式美化输出后的结果^[`nix derivation show nixpkgs#hello` 的执行结果]。
...
请注意其中被高亮的部分，那里记录的都是 Store 路径。

### inputDrvs

`inputDrvs` 是构建输入所对应的 Derivation。只有由这里指定的 Derivation 构建出来的产物，才能在沙箱环境中被使用。

`/nix/store/8brdx72cmmmjrap80d8x4k58f69h4247-stdenv-linux.drv` 是一个名为 **stdenv** 的特殊软件包的 Derivation。stdenv 是 Standard Environment（直译：标准环境）的缩写，其中打包了 `gcc`、`coreutils` 等在一般环境下必定会安装的多个基础软件包。

`/nix/store/y1j406svnfr5i2i78z4ybg8rg30ngmkj-bash-5.2p26.drv` 是 `bash` 的 Derivation，用作在沙箱环境中执行构建时的 shell。

`/nix/store/zm481qv2gsm86pb6c7k1sjhvhj3790kv-hello-2.12.1.tar.gz.drv` 是 hello 的**源代码**的 Derivation。事实上，Store 对象并不只有所谓的「软件包」。源代码、配置文件等一切被引入构建系统的文件，都会作为 Store 对象保存在 Nix Store 中。而且它们同样是由 Derivation 生成的。可以说，Nix 的构建系统是以 Derivation 为基本单位的。

### outputs

`outputs` 是构建产物的输出位置。可见，Store 路径是预先写在 Derivation 中的。

### builder

`builder` 是执行构建的 shell 可执行文件的路径。这里指定的正是由 `inputDrvs` 中列出的 `bash` 的 Derivation 构建出来的可执行文件。由此可以看出，`inputDrvs` 中指定的内容确实会在构建中被使用。

<details>
<summary>bash 的 Derivation</summary>

下面是 bash 的 store derivation `/nix/store/y1j406svnfr5i2i78z4ybg8rg30ngmkj-bash-5.2p26.drv` 以 JSON 美化输出后的结果。请注意高亮的部分：`hello` 的 `builder` 所指的 `/nix/store/4vzal97iq3dmrgycj8r0gflrh51p8w1s-bash-5.2p26/bin/bash`，与下面 `bash` 的 `outputs` 中的 Store 路径完全一致。
...
`env` 中指定的是传递给沙箱环境的环境变量。正如沙箱一章所述，在构建环境中只有这里指定的环境变量才会生效。

## Closure（闭包）^[[3.6. Closures - Nix Pills](https://nixos.org/guides/nix-pills/enter-environment#id1356)]

**Closure（闭包）** 是指从某个 Store 路径出发，能够直接或间接到达的全部 Store 路径的集合。

这里依然以 `hello` 为例。
`hello` 的 store derivation 的 Closure 等价于它的构建时依赖关系。

<details>
<summary>hello 的 store derivation 的 Closure</summary>

`nix-store --query --requisites /nix/store/y1j406svnfr5i2i78z4ybg8rg30ngmkj-bash-5.2p26.drv` 的执行结果。其中包含了 `bash`、`pkg-config` 等仅在构建时使用的软件包的 Derivation。
...
与之相对，`hello` 的 Derivation 的输出路径——也就是由 store derivation 构建出的 `hello` 软件包——它的 Closure 等价于运行时依赖关系。

<details>
<summary>输出路径的 Closure</summary>

`nix-store --query --requisites /nix/store/7bl684y3qpxrv01ird085rpf5kl6rk6f-hello-2.12.1/bin/hello` 的执行结果。其中包含了 `glibc` 等在 `hello` 运行时动态链接的共享库。
...
也就是说，Closure 表示的是软件包的「完整依赖关系树」。

## Realisation^[[Realisation - Zero to Nix](https://zero-to-nix.com/concepts/realisation)]

从 store derivation 构建出软件包的过程称为 **Realisation**（实体化）。

Realisation 的步骤如下：

1. 读取 `.drv`
2. 确认其 Closure 中的 Store 对象是否存在
   1. 从 Substituter 中查找该 Store 对象
   2. 如果不存在，则对该 Store 对象的 store derivation 执行 Realise
3. 执行构建
4. 将构建产物放入 Nix Store

Realisation 会递归地进行下去，直到 Closure 中的全部内容都被 realise 为止。

## Nix 表达式

store derivation 终究只是低层表示。实际用 Nix 构建软件包时，是从 Derivation 的高层表示——**Nix 表达式**（**Nix expression**）——生成 store derivation 的。下一章我们将介绍用于编写 Nix 表达式的编程语言：**Nix 语言**。
